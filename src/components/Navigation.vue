...
const handleThoughtAdded = (newThought) => {
    emit('thoughtAdded', newThought);  // Emit the new thought to the parent (App.vue)
    closeModal();  // Optionally close the modal after adding a thought
};
</script>

<template>
    <div class="fixed bottom-0 left-0 w-full bg-white h-16 border-t border-gray-300 md:hidden">
